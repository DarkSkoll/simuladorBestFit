...
public class Kernel extends Thread
{
  ...
  private Vector instructVector1 = new Vector();//nuevo vector
  private Vector segmentos = new Vector();
  private String status;
  private boolean doStdoutLog = false;
  private boolean doFileLog = false;
  public int runs;
  public int runcycles;
  public long block = (int) Math.pow(2,12);//4096
  public static byte addressradix = 10;

  public void crearSegmentos(){
    segmentos.addElement(new Segment(block, 2, 1));
    segmentos.addElement(new Segment(block, 3, 2));
    segmentos.addElement(new Segment(block, 6, 3));
    segmentos.addElement(new Segment(block, 9, 4));
    segmentos.addElement(new Segment(block, 12, 5));
  }
  ...
  public void init( String commands , String config )
  {
    ...
    long low = 0;
    long addr = 0;
    long addr1 = 0;//lector de segunda direccion
      ...
      while ((line = in.readLine()) != null)
      {
        if (line.startsWith("READ") || line.startsWith("WRITE"))
        {
          ...
          else
          {
            ...
            {
              tmp1 = st.nextToken();
              addr = Long.parseLong(tmp1,16);
              tmp1 = st.nextToken();
              addr1 = Long.parseLong(tmp1,16);//lectura de la segunda direccion
            }
            else
            {
            ...
            instructVector1.addElement(new Instruction(command,addr1)); //guardar segunda direccion y lectura
          }
        }
      }
      in.close();
  ...
  public void getSegmentacion(int segmentos[]){
    controlPanel.paintSeg(segmentos);
  }
  ...
  public void step()
  {
    ...
    Instruction instruct1 = ( Instruction ) instructVector1.elementAt( runs );// cargo intruccion de la segunda direccion
    ...
    getSegmentacion(Virtual2Physical.segmentacion( instruct.addr,instruct1.addr, virtPageNum , block ) );
    ...
    {
      Page page = ( Page ) memVector.elementAt( Virtual2Physical.pageNum( instruct.addr,virtPageNum , block ) );//llamado
      if ( page.physical == -1 )
      {
        if ( doFileLog )
        {
    ...
    PageFault.bestFit(instructVector, instructVector1, runs, segmentos);
    runs++;
  ...
  public void reset() {
	...
	//
     controlPanel.segment.setText("0");
    controlPanel.pagination.setText("0");
