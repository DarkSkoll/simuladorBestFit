...
public class PageFault {
  ...
  public static void bestFit(Vector inicioMemoria, Vector finMemoria, int runs, Vector segmentos){
    Instruction memInicio, memFinal;
    long tamaño;
    memInicio = (Instruction)inicioMemoria.elementAt(runs);
    memFinal = (Instruction)finMemoria.elementAt(runs);
    tamaño = memFinal.addr - memInicio.addr;
    Segment bestIdx = null;
    Segment tmp;
    for(int i = 0; i < segmentos.size(); i++){
      tmp = (Segment)segmentos.elementAt(i);
      if(tmp.size >= tamaño){
        if(bestIdx == null){
          bestIdx = tmp;
        }else{
          if(tmp.size < bestIdx.size){
            bestIdx = tmp;
          }
        }
      }
    ...
    if(bestIdx == null){
      System.out.println("Error: Proceso demasiado grande para cualquier segmento");
    }else{
      System.out.println("Asignado al Segmento " + bestIdx.numberOfSegment);
    }
  }
}
